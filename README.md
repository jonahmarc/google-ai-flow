...
> [!NOTE]
> The requirement states the app must work "without opening the browser or using their website directly." This is satisfied from the user's perspective: the user only interacts with `localhost:8000` — the local GUI. The Chrome debug window runs in the background as infrastructure, not something the user navigates. Playwright automates the Flow website invisibly, so the user never manually opens or touches `labs.google/fx/tools/flow`. If the requirement meant "no browser process at all," that would be impossible — reCAPTCHA tokens can only be generated by a live browser session and cannot be replicated externally.
